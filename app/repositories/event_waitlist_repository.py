...
class EventWaitlistRepository:
    ...
    def get_waitlist_for_event(event_id: int) -> List[EventWaitlist]:
        ...
        return EventWaitlist.query.filter_by(event_id=event_id).order_by(EventWaitlist.waitlisted_at.asc()).all()
    ...
    def remove_from_waitlist(event_id: int, user_id: int) -> bool:
        ...
        entry = EventWaitlist.query.filter_by(event_id=event_id, user_id=user_id).first()
        if entry:
