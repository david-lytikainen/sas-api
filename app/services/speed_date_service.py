...
class SpeedDateService:
    @staticmethod
    def generate_schedule(event_id: int, num_tables: int = 10, num_rounds: int = 10) -> bool:
        """
        ...
        try:
            ...
            if not attendees or len(attendees) < 2:
                current_app.logger.warning(f"Not enough attendees checked in for event {event_id} to generate schedule")
                return False
            ...
            if not males or not females:
                current_app.logger.warning(f"Need at least one person of each gender to generate schedule for event {event_id}")
                return False
            
            current_app.logger.info(f"Generating schedule for event {event_id} with {len(males)} males and {len(females)} females")
            ...
            speed_dates = SpeedDateMatcher.finalize_all_rounds(
                compatible_dates, id_to_user, event_id, calculated_num_tables, calculated_num_rounds
            )
            ...
            db.session.commit()
            current_app.logger.info(f"Generated {len(speed_dates)} speed dates for event {event_id}")
            return True
            
        except Exception as e:
            db.session.rollback()
            current_app.logger.error(f"Error generating schedule for event {event_id}: {str(e)}")
            return False
    ...
    def get_schedule_for_attendee(event_id: int, user_id: int) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            current_app.logger.error(f"Error getting schedule for user {user_id} in event {event_id}: {str(e)}")
            return []
    ...
    def get_all_schedules(event_id: int) -> Dict[int, List[Dict[str, Any]]]:
        ...
        try:
            ...
            for attendee in attendees:
                schedule = SpeedDateService.get_schedule_for_attendee(event_id, attendee.id)
                schedules[attendee.id] = schedule
            
            return schedules
        
        except Exception as e:
            current_app.logger.error(f"Error getting all schedules for event {event_id}: {str(e)}")
            return {} 