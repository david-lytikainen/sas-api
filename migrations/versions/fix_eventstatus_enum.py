...
def upgrade():
    ...
    op.execute(
        "CREATE TYPE eventstatus AS ENUM ('Registration Open', 'In Progress', 'Completed', 'Cancelled', 'Paused')"
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "price_per_person", sa.DECIMAL(precision=10, scale=2), nullable=False
        ),
...
def downgrade():
    ...
    op.execute("DROP TYPE eventstatus")
    op.execute(
        "CREATE TYPE eventstatus AS ENUM ('REGISTRATION_OPEN', 'IN_PROGRESS', 'COMPLETED', 'CANCELLED', 'PAUSED')"
    )
