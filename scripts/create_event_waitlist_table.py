...
def create_table():
    ...
    flask_config_name = os.getenv('FLASK_CONFIG') or 'default' # This might be used internally by create_app or its config loading
    
    print(f"Initializing Flask app (FLASK_ENV='{flask_env}', FLASK_CONFIG='{flask_config_name}' will be used if app configured to read them)...")
    app = create_app() # Call without arguments

    with app.app_context():
        print(f"Attempting to create table '{EventWaitlist.__tablename__}' if it doesn't exist...")
        try:
            ...
            if EventWaitlist.__tablename__ in inspector.get_table_names():
                print(f"Successfully verified that table '{EventWaitlist.__tablename__}' exists.")
            else:
                print(f"Verification failed: Table '{EventWaitlist.__tablename__}' does not appear to exist after creation attempt.")

        except Exception as e:
            print(f"An error occurred during table creation: {e}")
            print("Please ensure the database is running and accessible, and that prerequisite tables (users, events) exist if foreign key checks are immediate.")
